...
class BatchGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        if len(self.labels) > 1:
            ...
            # there is no class
            yolo_1 = np.zeros((r_bound - l_bound, 1*base_grid_h,  1*base_grid_w, len(self.anchors)//3, 4+1)) # desired network output 1
            yolo_2 = np.zeros((r_bound - l_bound, 2*base_grid_h,  2*base_grid_w, len(self.anchors)//3, 4+1)) # desired network output 2
            yolo_3 = np.zeros((r_bound - l_bound, 4*base_grid_h,  4*base_grid_w, len(self.anchors)//3, 4+1)) # desired network output 3
