...
class YoloLayer(Layer):
    def __init__(self, anchors, max_grid, batch_size, warmup_batches, ignore_thresh,
                    grid_scale, obj_scale, noobj_scale, xywh_scale, class_scale, class_exists,
                    **kwargs):
    ...
    def call(self, x):
        ...
        if self.class_exists:
            ...
            avg_cat     = tf.reduce_sum(object_mask) / (count + 1e-3)
        ...
        if self.class_exists:
            ...
            loss = tf.Print(loss, [grid_h, tf.reduce_sum(loss_xy),
                                       tf.reduce_sum(loss_wh),
                                       tf.reduce_sum(loss_conf)],  message='loss xy, wh, conf: \t',   summarize=1000)
...
def create_yolov3_model(
    ...
    if nb_class == 1:
        # nb_class is 1, no class loss is required
        class_exists = False
        nb_class = 0
    else:
        class_exists = True
    ...
    loss_yolo_1 = YoloLayer(anchors[12:],
                            ...
                            xywh_scale,
                            class_scale,
                            class_exists)([input_image, pred_yolo_1, true_yolo_1, true_boxes])
    ...
    loss_yolo_2 = YoloLayer(anchors[6:12],
                            ...
                            xywh_scale,
                            class_scale,
                            class_exists)([input_image, pred_yolo_2, true_yolo_2, true_boxes])
    ...
    loss_yolo_3 = YoloLayer(anchors[:6],
                            ...
                            xywh_scale,
                            class_scale,
                            class_exists)([input_image, pred_yolo_3, true_yolo_3, true_boxes])
